...
parser.add_argument('-t','--threads',required=False,type=float,default=4,help="threads") 
...
parser.add_argument('-i','--input',required=True,type=str,help="name of input file (either expression matrix in .csv, .tsv, .txt format or 10X directory containing barcodes.tsv, features.tsv, matrix.mtx, and tissue_positions_list.csv)")   
...
operator = STITCH(i,n_clusters=n_clusters,num_states=3,normal_spots=normal_spots,beta_spots = beta_spot,nthreads=nthreads,gene_mapping_file_name=gene_mapping_file_name)
